...
function ubah() {
    ...
    if ($("#profile input[type='file']")[0].files[0] == undefined) {
        ...
        formData.append(
            "attachment",
            $("#profile input[type='file']")[0].files[0]
        );
        formData.append("full_name", $("#fullName").val());
        formData.append("company", $("#company").val());
        formData.append("country", $("#Country").val());
    ...
    loadStart();
    $.ajax({
        url: `${urlApi}profile/update/${localStorage.getItem("user")}`,
        type: "POST",
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            Authorization: "Bearer " + localStorage.getItem("token"),
        ...
        success: function (response) {
            setTimeout(function () {
                ...
                Swal.fire({
                    ...
                    window.location = `${baseUrl}be-admin/settings`;
                });
            }, 1000);
        },
        error: function (xhr) {
